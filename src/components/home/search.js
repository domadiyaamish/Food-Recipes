...
export default function Search() {
  ...
  const [handleInputChange, setHandleInputChange] = useState(null);
  ...
  const handleSearch = async (e) => {
    ...
    fetch("https://localhost:44329/api/Recipes/search/"+handleInputChange)
    .then((res) => res.json())
    .then((data) => {
      setRecipies(data);
      console.log("amish");
...
<div style={{ marginLeft:"80%" }}>
  <Form onSubmit={handleSearch}>
  <Form.Group className="mb-3 col-lg-3" controlId="formBasicEmail" style={{ display: "flex", justifyContent: "flex-end", margin:"40px" ,marginRight:"200px"}}>
                                        <Form.Control type="text" onChange={(e) => { setHandleInputChange(e.target.value) }} style={{height:"40px", width:"300px"}} placeholder="Recipe Name" name="RecipeName" required />

                                        <Button variant="primary" className='mb-3 col-lg-3.5' style={{ backgroundColor: "#FFB900",marginRight:"10px",marginLeft:"10px" }} type="submit">
                                        Search
                                    </Button>
                                    </Form.Group>
